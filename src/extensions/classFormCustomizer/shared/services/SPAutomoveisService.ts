...
export default class SPAutomoveisService {
   private _sp: SPFI;
   ...
   public async deleteItem(listGUID: any, itemID: number): Promise<void> {
      try {
         await this._sp.web.lists.getById(listGUID).items.getById(itemID).delete();
      } catch (err) {
         console.error(err);
      }
   }
